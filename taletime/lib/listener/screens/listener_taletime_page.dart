...
class ListenerTaletimePage extends StatefulWidget {
  ...
  final List<QueryDocumentSnapshot<Story>> docs;

  const ListenerTaletimePage(this.docs, this.buttons,
      {super.key});
...
class _ListenerTaletimePageState extends State<ListenerTaletimePage> {
  ...
  List<StoryActionButton> getButtons(Story story, DocumentReference<Story> ref) {
    if (widget.buttons is FavoriteActionButton) {
      return (widget.buttons as FavoriteActionButton).build(context, ref as DocumentReference<AddedStory>);
    } else {
      return (widget.buttons as AddStoryActionButtons).build(context,story);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    List<DocumentReference<Story>> storiesDocumentSnapshotRefs =
    widget.docs.map((e) => e.reference).toList();
    List<Story> storiesDocumentSnapshot = widget.docs.map((e) => e.data()).toList();
                        ...
                        itemBuilder: (k, i) {
                          return Padding(
                            ...
                            child: Column(
                              children: [
                                StoryListItem(
                                  buttons: getButtons(storiesDocumentSnapshot[i], storiesDocumentSnapshotRefs[i]),
